...
2. **Allow user to filter contacts by reminder time:** Currently, reminders and the `find` command only interact via the text they contain as a note and the alphanumeric value of the time it was set to. While this allows users to search for contacts with reminders on a specific date, we plan to improve functionality of the `find` function to filter for contacts before and/or after a specified date/time as well.
